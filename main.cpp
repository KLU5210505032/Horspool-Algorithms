...
int Horspool(string kelimedizisi, string metindizisi) {
    ...
    while (i < b) {
        ...
        i+=tablo[metindizisi[i]];//Bu satırda horspool algoritmasının skip mekanizması var.
                           // Bu mekanizma, eğer belirlenemeyen bir karakter varsa, metindeki bir sonraki olası eşleşme pozisyonunu belirlemek için kullanılır.
    }
    return sayac;
}

int main() {
    string dosya1 = "alice_in_wonderland.txt";//txt dosyasını aynı klasörde kullandım ve direkt dosyayı çağırdım .
    string dizi[] = {"upon", "sigh","Dormouse","jury-box","swim"};
    FILE* dosya;
    dosya = fopen(dosya1.c_str(), "r");
